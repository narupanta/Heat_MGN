...
def moving_heat_source(x, x_center, y_center):
    ...
    return q_max* np.exp(-3 * (x[0] - x_center)**2/radius**2 + -3 * (x[1] - y_center)**2 / radius**2) \
...
while t < T_total:
    ...
    tracks = [(1, 0), (0, 1), (-1, 0), (0, -1)]
    quadrants = [(-1,-1), (1,-1), (1,1), (-1,1)]
    total_duration = track_duration * len(tracks)
    r0 = (cube.length/2, cube.width/2)
    dr = (cube.length/4, cube.width/4)
    if t < total_duration:
        current_segment = int(t // track_duration)
        local_t = t - current_segment * track_duration
        x_center = source_speed * local_t
        y_center = source_speed * local_t
        q.x.array[:] = np.array([moving_heat_source(x, r0[0] + quadrants[current_segment][0] * dr[0] + x_center * tracks[current_segment][0], r0[1] + quadrants[current_segment][1] * dr[1] + y_center * tracks[current_segment][1]) for x in domain.geometry.x], dtype=PETSc.ScalarType)
    else :
        q.x.array[:] = np.array([0.0 for _ in domain.geometry.x], dtype=PETSc.ScalarType)
