...
#     def _mean(self):
#         safe_count = torch.maximum(self._acc_count.to(device), torch.tensor([1.], device=device))
#         return self._acc_sum.to(device) / safe_count

#     def _std_with_epsilon(self):
#         safe_count = torch.maximum(self._acc_count.to(device), torch.tensor([1.], device=device))
#         std = torch.sqrt(self._acc_sum_squared.to(device) / safe_count - self._mean() ** 2)
#         return torch.maximum(std, self._std_epsilon.to(device))
